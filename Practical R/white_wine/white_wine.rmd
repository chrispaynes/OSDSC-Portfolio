...
#### Inspect the Data.Frame structure
The Dataset contains 4,898 white wines and with 11 variables on quantifying the chemical properties of each wine.
Comparing the number rows and columns captured by R against the number of rows and columns in the CSV reveals we've successfully read the entire file contents.
wine experts rated the quality of each wine, providing a rating between 0 (very bad) and 10 (very excellent).
All columns except for "X" and "quality" contain double-precision floating point numbers (decimals).
...
- There's a nearly identical amount of Below Average and Exceptional wines.
- The majority of wines fall within the Above Average quality category followed by the Average quality category wines.'
- The variables (excluding the ratios) with the largest ranges are total.sulfur.dioxide, free.sulfur.dioxide and residual.sugar. This may indicate a likelihood for outliers in these variables.
``` {r}
summary(white_wine)
...
#### Chlorides Histogram
Transformed the data to remove the long tails and reveal what can best be described as a unimodal distribution with a peak around 0.0425.
```{r}
...
Transformed the data to remove most of the long tails and reveal a mostly unimodal distribution.
The majority of the Total Sulfur Dioxide values lie between 108.0 and 167.0.
```{r}
...
Transformed the data to remove the long tails and reveal a unimodal distribution. The majority of the Quality values lie between 5 and 6. All values here are integers.
```{r}
...
Transformed the data to remove the long tails and reveal a unimodal distribution.
The majority of the Alcohol values lie between 20.628 and 33.000.
```{r}
...
Reveals a long tailed unimodal distribution. The
majority of the Residual Sugar To Sulfates Level values lie between 3.571 and 21.244 .
```{r}
...
There are too many variables at play in the above Correlation Chart, let's focus on variables with a strong correlation with Wine Quality.
- There's a strong positive or negative correlation between Alcohol and most other variables except for Volatile Acidity.
- Strongest positive correlation between Wine Quality is that of Alcohol.
- Strongest negative correlation between Wine Quality is that of Wine Density.
```{r}
...
there's a narrower alcohol range for Quality 8 wines
Above Average quality wines have the largest Alcohol content range
Average quality wines tended to have an Alcohol content range between 9 and 10
Exceptional Quality Wines had an ABV between 11 and 13
```{r}
ggplot(aes(
  y = quality,
  x = alcohol
...
- mean density appears to decrease as the wine quality increases
- average quality wine has a largest variability in density
- Above Average wine has the smallest variability in density
```{r}
ggplot(aes(
  y = quality,
  x = density
...
data = white_wine
  ...
  scale_y_continuous(breaks = seq(0, 10)) +
  scale_x_continuous(breaks = seq(floor(min(white_wine$density)), max(white_wine$density), 0.005)) +
  facet_wrap(~quality_lev_f, scales = "free") +
...
- the Total Sulfur Dioxide level appears to decrease as the quality increases
- all quality levels have a large range in values, however the maximum Total Sulfur Dioxide for Exceptional wines is much lower than that of lower quality wines.
```{r}
ggplot(aes(
  y = quality,
...
- the Chloride level decreases as the wine quality increases
- the Chloride range between the different quality levels is pretty narrow. Despite its lower ranking in the Top 5 R-Squared values, this narrow range may indicate that the amount of Chlorides goes a long way in affecting a wine's quality
```{r}
ggplot(aes(
  y = quality,
  x = chlorides
...
- similar Volatile Acidity between Below Average and Exceptional Wines
- Above Average Wines have a similar Volatile Acidity of around 0.25
- Excluding Quality 3 and 9 Wines, all quality levels seemed to have a similar range in values. However, I would assume with more data points for Quality 3 and 9 Wines, we'd likely see similarly large range.
```{r}
ggplot(aes(
  y = quality,
  x = volatile.acidity
...
- not a significant difference in pH values between wine quality levels with most values being between 3.1 and 3.25 pH
- average and Above Average wines had the largest pH ranges
```{r}
ggplot(aes(
  y = quality,
  x = pH
...
- Exceptional Quality Wines and Quality 3 Wines have the smallest range of Citric Acid values
- Average and Above Average Wines have the largest range of Citric Acid values
...
ggplot(white_wine, aes(quality, citric.acid)) +
  ...
  ggtitle("Quality and Citric Acid Distribution Averages")
```
...
Average quality wines tend to be between 9% and 11% ABV and ABV tends to increase linearly with quality
```{r}
ggplot(aes(
  x = quality,
  y = alcohol
...
#### Marginal Histograms
scatterhist” in Matlab, apparently. The seaborn version also shows the strength of the linear relationship between the x and y variables.
...
- linear model indicating that the alcohol level generally increases as the quality increases
- there significantly more Quality 8 Exceptional Wines than there are Quality 9 Exceptional Wines
- the Quality 9 wines are most likely to be around 13% ABV.
- Qualities 4-7 have a generally even distribution of alcohol levels ranging from 8.5% to 13%
- the majority of the Above Average Quality wines are of Quality 6 at 11% ABV
```{r}
WVPlots::ScatterHist(
  frame = white_wine,
  yvar = "alcohol",
...
##### Below Average and Exception Wine Quality Compared to Alcohol and Wine Density
...
ggMarginal(
  ggplot(
    ...
    theme_bw(15) +
    ggtitle(
      "Below Average and Exception Wine Quality",
...
##### Below Average and Exception Wine Quality Compared to Total Sulfur Dioxide and Chlorides
...
##### Below Average and Exception Wine Quality Compared to Total Sulfur Dioxide and Volatile Acidity
- there's a lot of similarity between Total Sulfur Dioxide and Volatile Acidity for both Below Average and Exceptional Wines
- Exceptional wines tended to have slightly more clustering in their values, but that difference between Below Average wines is negligible
```{r}
ggMarginal(
  ggplot(
    data = white_wine[white_wine$quality_lev_f %in% c("below avg.", "Exceptional"), ],
...
```{r}
ggplot(white_wine[white_wine$quality > 7, ], aes(alcohol, residual.sugar, colour = quality)) +
  geom_point(
    position = position_jitter(height = 0),
    alpha = 1 / 1
  ) +
