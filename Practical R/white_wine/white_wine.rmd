...
```{r}
ggplot(white_wine[white_wine$quality_lev_f %in% c("below avg.", "exceptional"), ], aes(pH, citric.acid)) +
  geom_point(aes(colour = quality_lev_f, alpha = 1 / 10)) +
  scale_y_continuous(limits = quantile(white_wine$citric.acid, probs = c(0.05, .97))) +
  scale_x_continuous(
    limits = quantile(white_wine$pH, probs = c(0.05, .95)),
    breaks = seq(floor(min(white_wine$pH)), max(white_wine$pH), 0.1)
  ) +
  theme(legend.position = c(0.1, 0.75)) +
  labs(x = "pH", y = "Citric Acid Concentration") +
  guides(colour = guide_legend(title = "Wine Quality Levels", override.aes = list(alpha = 1))) +
  ggtitle("Below Average and Exceptional Wine Quality and the Relationship Between pH and Citric Acid")
```
...
```{r}
ggplot(white_wine[white_wine$quality_lev_f %in% c("below avg.", "exceptional"), ], aes(alcohol, density, color = quality_lev_f, alpha = 1 / 5)) +
  ...
  ggtitle("Below Average and Exceptional Wine Quality and the Relationship Between Alcohol and Wine Density")
```
...
```{r}
ggplot(white_wine[white_wine$quality_lev_f %in% c("below avg.", "exceptional"), ], aes(alcohol, residual.sugar, colour = quality_lev_f)) +
  geom_point(position = position_jitter(height = 0)) +
  scale_y_continuous(limits = quantile(white_wine$residual.sugar, probs = c(0.1, 0.9)), ) +
  scale_x_continuous(
    limits = quantile(white_wine$alcohol, probs = c(0.1, 0.95)),
    breaks = seq(9.5, max(white_wine$alcohol), 0.5)
  ) +
  labs(x = "Alcohol", y = "Residual Sugar") +
  guides(colour = guide_legend(title = "Wine Quality Levels", override.aes = list(alpha = 1))) +
  ggtitle("Below Average and Exceptional Wine Quality and the Relationship Between Alcohol and Residual Sugar")
```
